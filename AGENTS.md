...
- `ts/`: TypeScript reference implementation (npm `@yea-protocol/sdk`). Zero runtime dependencies. `src/` holds the core, `src/cli.ts` the CLI, `src/tools.ts` + `src/mcp.ts` the MCP bridge, and `src/openapi.ts` the OpenAPI adapter.
- `cli/`: the `yea` command (npm `@yea-protocol/cli`), a thin package over the SDK's `./cli` export. The CLI's code lives in `ts/src/cli.ts`.
