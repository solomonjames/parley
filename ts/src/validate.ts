...
function checkType(type: string, v: unknown): string | null {
  ...
  if (base.endsWith('[]')) {
    ...
    for (const x of v)
      if (checkType(base.slice(0, -2), x))
        return `an array of ${base.slice(0, -2)}`;
  ...
  switch (base) {
    ...
    case 'int':
      return Number.isInteger(v) ? null : 'an integer';
    case 'number':
    ...
    case 'date':
      return typeof v === 'string' && DATE.test(v)
        ? null
        : 'a date like 2026-09-24';
    case 'datetime':
      return typeof v === 'string' && DATETIME.test(v)
        ? null
        : 'an ISO datetime like 2026-09-24T15:00:00Z';
    case 'any':
      return null;
    default:
      if (base.includes('|')) {
...
export function validateParams(
  ...
  for (const [raw, type] of Object.entries(schema)) {
    ...
    if (v === undefined || v === null) {
      if (!optional)
        problems.push(
          `missing \`${path}${name}\` (${typeof type === 'string' ? type : 'object'})`,
        );
    ...
    if (typeof type === 'string') {
      ...
      if (want)
        problems.push(
          `\`${path}${name}\` must be ${want} (got ${JSON.stringify(v)})`,
        );
    } else if (Array.isArray(type)) {
      if (!Array.isArray(v))
        problems.push(`\`${path}${name}\` must be an array of objects`);
      else
        v.forEach((item, i) => {
          try {
            if (
              ...
              throw new Error(`\`${path}${name}.${i}\` must be an object`);
