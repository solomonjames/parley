...
const errFrame = (message: string, code = "bad_frame") => JSON.stringify({ parley: 1, id: "s_err", re: "?", kind: "ERROR", code, message }) + "\n";
  ...
  input.on("data", (chunk: string) => {
    ...
    for (;;) {
      ...
      if (inflight >= MAX_INFLIGHT) {
        send(JSON.stringify({ parley: 1, id: "s_busy", re: typeof frame?.id === "string" ? frame.id : "?", kind: "ERROR", code: "limit", message: `more than ${MAX_INFLIGHT} requests in flight on this connection`, retry: 1 }) + "\n");
        continue;
      }
      inflight++;
      svc
        ...
        .catch(() => send(JSON.stringify({ parley: 1, id: "s_err", re: typeof frame?.id === "string" ? frame.id : "?", kind: "ERROR", code: "internal", message: "reply could not be serialized" }) + "\n"))
        .finally(() => inflight--);
    }
  });
}
