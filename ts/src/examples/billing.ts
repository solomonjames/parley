...
export function billing(opts: {
  ...
  return service({
      ...
      plan: ({ params }) =>
        pick(params.who, (c) => {
          ...
          const refund = (amount: number, why: string): Plan => ({
            ...
            apply: () => (
              (pay.refunded += amount),
              { refunded: usd(amount), payment: pay.id }
            ),
          ...
          if (params.usd != null) {
            ...
            if (amount <= 0 || amount > left)
              throw new ParleyError(
                ...
                {
                  fix: [
                    fix(`refund the rest (${usd(left)})`, { usd: left / 100 }),
                  ],
                },
              );
          ...
          return current
            ? [
                refund(left, 'full'),
                refund(unused, `unused ${Math.round(periodLeft(c) * 30)} days`),
              ]
            : refund(left, 'full');
        }),
    })
    .intent('billing.change_plan', {
      ...
      plan: ({ params }) =>
        pick(params.who, (c) => {
          const to = params.plan as PlanId,
            from = c.plan;
          ...
          const later: Plan = {
            ...
            apply: () => (
              (c.nextPlan = to), { plan: to, from: date(c.renews) }
            ),
            revert: () => {
              c.nextPlan = null;
            },
          ...
          return [
            {
              ...
              undoWindow: Math.floor((c.renews - now) / 1000),
              apply: () => (
                (c.cancelAt = c.renews), { cancels: date(c.renews) }
              ),
              revert: () => {
                c.cancelAt = null;
              },
