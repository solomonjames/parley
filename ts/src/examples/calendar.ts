...
export function calendar(opts: {
  ...
  const overlaps = (s: number, e: number, skip?: string) =>
    events.find(
  ...
  const freeSlots = (day: string, minutes: number, skip?: string) => {
    ...
    for (
      ...
      if (!overlaps(t, t + minutes * 60_000, skip)) out.push(iso(t));
  ...
  return service({
    ...
    .ask('calendar.free', {
      ...
      run: ({ params }) => ({
        day: params.day,
        slots: freeSlots(params.day, params.minutes ?? 30),
      }),
    })
        ...
        pick(params.event, (e) => {
          ...
          if (params.to) {
            ...
            if (s < Date.now())
              throw new ParleyError(
                'invalid_params',
                `\`to\` is in the past (${params.to})`,
                {
                  fix: [
                    fix('use a future time', { to: iso(s + 365 * 86400_000) }),
                  ],
                },
              );
            ...
            if (clash) {
              const alts = freeSlots(
                params.to.slice(0, 10),
                dur / 60_000,
                e.id,
              ).slice(0, 3);
              ...
              throw new ParleyError(
                'conflict',
                `${params.to} overlaps "${clash.title}"`,
        ...
        pick(params.event, (e) => ({
          ...
          effects: [
            ...
            ...e.with.map((w) =>
              send(
                w,
                params.note ? `cancellation: ${params.note}` : 'cancellation',
              ),
            ),
          ],
    ...
    .intent('calendar.book', {
      ...
      plan: ({ params }) => {
        ...
        return freeSlots(params.day, minutes)
          .slice(0, 3)
          .map((t): Plan => {
            const ev: Ev = {
              id: `e${++seq}`,
            ...
            return {
              ...
              effects: [
                ...
                ...ev.with.map((w: string) => send(w, 'invite')),
              ],
              undoWindow: 86400,
              apply: () => {
                events.push(ev);
