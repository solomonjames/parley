...
export type Kind = "BRIEF" | "ANSWER" | "PROPOSALS" | "CLARIFY" | "RECEIPT" | "ERROR" | "EVENT";
export type Risk = "low" | "medium" | "high";
export type EffectOp = "create" | "update" | "delete" | "send" | "charge" | "other";
export type Scalar = string | number | boolean | null;

export type ErrorCode =
  | "bad_frame" | "unknown_capability" | "invalid_params" | "unauthorized" | "forbidden"
  | "consent_required" | "not_found" | "expired" | "conflict" | "limit" | "unavailable" | "internal";
...
export interface More { handle: string; path: string; remaining: number; est: number }
...
export type ParamSchema = { [name: string]: string | ParamSchema | [ParamSchema] };

export interface Proof { key: string; ts: number; sig: string }

export interface Fix { say: string; params?: Record<string, unknown> }
...
interface FrameBase { parley: 1; id: string }
...
interface ReplyBase extends FrameBase { re: string; lens?: string }
export interface Brief extends ReplyBase { kind: "BRIEF"; service: { id: string; name: string; summary: string }; capabilities: CapabilityInfo[]; more?: More[] }
...
export interface Event extends ReplyBase { kind: "EVENT"; message: string; progress?: number; data?: unknown }
export type Reply = Brief | Answer | Proposals | Clarify | ReceiptReply | ErrorReply | Event;
