...
export class MemoryHandleStore implements HandleStore {
  ...
  put(h: string, v: Parked) {
    ...
    if (this.m.size > 10_000)
      for (const [k, e] of this.m) if (e.exp < Date.now()) this.m.delete(k);
  }
...
export function fit<R extends Reply>(
  ...
  const moreFor = (handles: boolean): More[] => {
    ...
    for (const { path, kept, handle } of cut.values()) {
      ...
      const rest =
        typeof full === 'string' ? full.slice(kept) : full.slice(kept);

      if (handles)
        store.put(
          handle,
          typeof rest === 'string'
            ? { kind: 'text', text: rest, owner }
            : { kind: 'array', items: rest, owner },
        );
      ...
      out.push({
        ...
        est: typeof rest === 'string' ? est(rest) : est(lean(rest)),
      });
    }
  ...
  for (let guard = 0; guard < 200; guard++) {
    ...
    if (typeof cur === 'string') {
      ...
      cut.set(key, {
        path: pick.path,
        kept: alreadyCut ? Math.min(prevKept, kept) : kept,
        handle: cut.get(key)?.handle ?? randomId('h'),
      });
