...
import type {
  Brief, CapabilityInfo, Effect, ErrorReply, Event, FinalReply, Money, ParamSchema, Proposal, Receipt, ReceiptReply, Request, Risk, Verb,
} from "./types.js";
...
export interface Clarification { clarify: { question: string; options: { label: string; params: Record<string, unknown> }[] } }

export const clarify = (question: string, options: { label: string; params: Record<string, unknown> }[]): Clarification => ({ clarify: { question, options } });
...
export class Service {
  ...
  get capabilities(): CapabilityInfo[] {
    ...
    for (const [name, d] of this.asks) out.push({ name, kind: "ask", summary: d.summary, ...(d.params ? { params: d.params } : {}) });
    for (const [name, d] of this.intents) out.push({ name, kind: "intent", summary: d.summary, ...(d.params ? { params: d.params } : {}), ...(d.risk ? { risk: d.risk } : {}) });
    return out;
  }

  brief(budget = this.opts.defaultBudget ?? 2000, re = "discover"): Brief {
    const r: Brief = { parley: 1, id: randomId("s", 6), re, kind: "BRIEF", service: { id: this.opts.id, name: this.opts.name, summary: this.opts.summary }, capabilities: this.capabilities };
    return fit(r, budget, this.handles);
  }

  /** Handle one request frame. EVENTs go to `emit`; the final reply is returned. */
  async handle(frame: unknown, emit: (e: Event) => void = () => {}): Promise<FinalReply> {
    const req = frame as Request;
    const re = typeof (req as any)?.id === "string" ? req.id : "?";
    try {
      if (!req || typeof req !== "object" || (req as any).parley !== 1 || typeof req.id !== "string") {
        throw new ParleyError("bad_frame", 'frames need "parley": 1 and a string "id"');
      }
      const budget = Number.isInteger(req.budget) && req.budget! > 0 ? req.budget! : this.opts.defaultBudget ?? 2000;
      switch (req.verb) {
  ...
  private errorReply(re: string, e: unknown): ErrorReply {
    if (e instanceof ParleyError) return { parley: 1, id: randomId("s", 6), re, kind: "ERROR", code: e.code, message: e.message, ...e.extra };
    this.opts.onError?.(e);
    return { parley: 1, id: randomId("s", 6), re, kind: "ERROR", code: "internal", message: "the service failed unexpectedly", retry: 5 };
  }

  private unknownCapability(name: unknown, kind: "ask" | "intent"): never {
    const all = [...this.asks.keys(), ...this.intents.keys()];
    const other = kind === "ask" ? this.intents.has(String(name)) : this.asks.has(String(name));
    if (other) {
      const verb = kind === "ask" ? "INTENT" : "ASK";
      throw new ParleyError("unknown_capability", `${name} is ${kind === "ask" ? "an intent" : "an ask"} capability`, { fix: [fix(`send it with ${verb}`)] });
    }
    const near = closest(String(name), all);
    throw new ParleyError("unknown_capability", `no capability named ${JSON.stringify(name)}`, {
      fix: near ? [fix(`did you mean ${near}?`)] : [fix(`send HELLO to list capabilities (${all.length} available)`)],
    });
  }
  ...
  private async authorize(req: Request, verb: Verb, capability: string, target: string, proposal?: Proposal): Promise<(GrantCheck & { ok: true }) | null> {
    ...
    if (!grants.length) {
      if (required) throw new ParleyError("unauthorized", `${verb} needs a grant from your principal`, { fix: [fix("ask your principal to issue a grant (parley grant) and send it in `grants` with a `proof`")] });
      return null;
    }
    const proofErr = await checkProof(req.proof, { aud: this.id, verb, target }, this.now());
    if (proofErr) throw new ParleyError("unauthorized", proofErr, { fix: [fix(`sign {aud:"${this.id}",verb:"${verb}",target,ts} with the grant holder key`)] });
    const checks: GrantCheck[] = [];
    for (const g of grants) {
      const c = await checkGrant(g, {
        service: this.id, verb, capability, now: this.now(), trusted: this.opts.trust ?? [], proofKey: req.proof!.key,
    ...
    if (forbidden && !forbidden.ok) throw new ParleyError("forbidden", forbidden.reason, { need: forbidden.need });
    const first = checks[0] as GrantCheck & { ok: false };
    throw new ParleyError("unauthorized", first.reason);
  }

  private async onAsk(req: Request & { verb: "ASK" }, budget: number): Promise<FinalReply> {
    const def = this.asks.get(req.capability) ?? this.unknownCapability(req.capability, "ask");
  ...
  private async planIntent(def: IntentDef, req: Request & { verb: "INTENT" }, budget: number, emit: (e: Event) => void, principal: string | null): Promise<FinalReply> {
    ...
    if (!plans.length) throw new ParleyError("not_found", "no way to satisfy this intent", { fix: [fix("relax the constraints and try again")] });
    const now = this.now();
  ...
  private execute(stored: StoredProposal, auth: GrantCheck & { ok: true }, reqId: string, emit: (e: Event) => void): Promise<ReceiptReply | ErrorReply> {
    ...
    const run = (async (): Promise<ReceiptReply | ErrorReply> => {
      const ctx: CommitCtx = {
        principal: auth.iss,
        progress: (message, progress, data) => emit({ parley: 1, id: randomId("s", 6), re: req.id, kind: "EVENT", message, ...(progress !== undefined ? { progress } : {}), ...(data !== undefined ? { data } : {}) }),
      };
      try {
        ...
        const receipt: Receipt = {
          ...
          effects: proposal.effects, cost: proposal.cost, undo: proposal.undo ? { until: at + proposal.undo.window } : null,
          ...(result !== undefined ? { result } : {}),
        };
        this.receipts.set(receipt.id, { receipt, plan, result, principal: auth.iss });
  ...
  private async onUndo(req: Request & { verb: "UNDO" }, budget: number, emit: (e: Event) => void): Promise<FinalReply> {
    ...
    if (!stored || stored.receipt.undoes) throw new ParleyError("not_found", `no undoable receipt ${JSON.stringify(req.receipt)}`);
    const auth = (await this.authorize(req, "UNDO", stored.receipt.capability, req.receipt))!;
    ...
    if (this.now() > receipt.undo.until) throw new ParleyError("expired", `the undo window closed at ${new Date(receipt.undo.until * 1000).toISOString()}`);
    stored.undone = (async (): Promise<ReceiptReply | ErrorReply> => {
      try {
        await plan.revert!({
          principal: auth.iss, result: stored.result,
          progress: (message, progress) => emit({ parley: 1, id: randomId("s", 6), re: req.id, kind: "EVENT", message, ...(progress !== undefined ? { progress } : {}) }),
        });
        const inverse: Record<string, Effect["op"]> = { create: "delete", delete: "create", update: "update", send: "other", charge: "other", other: "other" };
        const effects: Effect[] = receipt.effects.map((e) =>
          e.op === "update" ? { ...e, from: e.to, to: e.from } : e.op === "charge" ? { op: "other", target: e.target, detail: "refund" } : e.op === "send" ? { op: "other", target: e.target, detail: "cannot unsend; follow-up sent if supported" } : { ...e, op: inverse[e.op] });
        const undo: Receipt = { id: randomId("r", 6), proposal: receipt.proposal, capability: receipt.capability, summary: receipt.summary, at: this.now(), effects, cost: null, undo: null, undoes: receipt.id };
        return { parley: 1, id: randomId("s", 6), re: req.id, kind: "RECEIPT", receipt: undo };
      } catch (e) {
...
export const update = (target: string, field: string, from: Effect["from"], to: Effect["to"], detail?: string): Effect => ({ op: "update", target, field, from, to, ...(detail ? { detail } : {}) });
export const remove = (target: string, detail?: string): Effect => ({ op: "delete", target, ...(detail ? { detail } : {}) });
export const send = (target: string, detail?: string): Effect => ({ op: "send", target, ...(detail ? { detail } : {}) });
export const charge = (target: string, detail?: string): Effect => ({ op: "charge", target, ...(detail ? { detail } : {}) });
