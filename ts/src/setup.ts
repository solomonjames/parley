...
const START = "<!-- PARLEY_START -->", END = "<!-- PARLEY_END -->";
...
export function writeBlock(file: string): string {
  ...
  const head = !cur && file.endsWith(".mdc") ? "---\ndescription: Using Parley tools on the user's behalf\nalwaysApply: true\n---\n\n" : "";
  const next = re.test(cur) ? cur.replace(re, AGENT_BLOCK) : head + cur + (cur && !cur.endsWith("\n") ? "\n" : "") + (cur ? "\n" : "") + AGENT_BLOCK;
...
// ---- clients ----
interface Scope { local: boolean; cwd: string }
interface Target {
  name: string;
  detect(): boolean;
...
function jsonTarget(name: string, file: (s: Scope) => string, detect: () => boolean, block?: (s: Scope) => string, key = "mcpServers", extra: Record<string, unknown> = {}): Target {
  return {
    name,
    detect,
    ...
    install: (s) => {
      ...
      const b = block?.(s);
      return [`MCP server "parley" → ${f}`, ...(b ? [`agent instructions → ${writeBlock(b)}`] : [])];
    },
    uninstall: (s) => {
      const out: string[] = [];
      const f = file(s);
...
export const CLIENTS: Record<string, Target> = {
  ...
  "claude-code": jsonTarget("Claude Code", (s) => (s.local ? join(s.cwd, ".mcp.json") : join(homedir(), ".claude.json")), () => has("claude") || existsSync(join(homedir(), ".claude")),
    (s) => (s.local ? join(s.cwd, "CLAUDE.md") : join(homedir(), ".claude", "CLAUDE.md")), "mcpServers", { type: "stdio", alwaysLoad: true }),
  ...
  codex: {
    ...
    installed: () => existsSync(join(homedir(), ".codex", "config.toml")) && /^\[mcp_servers\.parley\]/m.test(readFileSync(join(homedir(), ".codex", "config.toml"), "utf8")),
    install: (s) => {
      ...
      out.push(`agent instructions → ${writeBlock(s.local ? join(s.cwd, "AGENTS.md") : join(homedir(), ".codex", "AGENTS.md"))}`);
      return out;
    },
    uninstall: (s) => {
      const file = join(homedir(), ".codex", "config.toml");
      ...
      if (existsSync(file)) {
        ...
        if (i >= 0) {
          ...
          next = [...lines.slice(0, i), ...lines.slice(j)].join("\n").replace(/\n{3,}/g, "\n\n");
        }
        if (next !== cur) (writeFileSync(file, next), out.push(`removed [mcp_servers.parley] from ${file}`));
      }
      const agents = s.local ? join(s.cwd, "AGENTS.md") : join(homedir(), ".codex", "AGENTS.md");
