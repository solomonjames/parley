...
export function canonical(v: unknown): string {
  ...
  switch (typeof v) {
    case "boolean":
      return v ? "true" : "false";
    case "number":
      if (!Number.isSafeInteger(v)) throw new TypeError(`canonical JSON allows only safe integers, got ${v}`);
      return String(v);
    case "string":
      return quote(v);
    case "object": {
      ...
      const keys = Object.keys(obj).filter((k) => obj[k] !== undefined).sort(byCodePoint);
      return "{" + keys.map((k) => quote(k) + ":" + canonical(obj[k])).join(",") + "}";
    }
    default:
