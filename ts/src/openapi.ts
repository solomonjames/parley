...
const slug = (s: string) => s.toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_|_$/g, "") || "api";
...
function resolve(spec: Json, v: any, depth = 0): any {
  ...
  if (typeof v.$ref === "string" && v.$ref.startsWith("#/")) {
    const target = v.$ref.slice(2).split("/").reduce((o: any, k: string) => o?.[k.replace(/~1/g, "/").replace(/~0/g, "~")], spec);
...
function typeOf(spec: Json, schema: any, depth = 0): string {
  ...
  if (Array.isArray(s.enum) && s.enum.length && s.enum.length <= 12 && s.enum.every((x: unknown) => typeof x === "string" && !x.includes("|"))) return s.enum.join("|");
  switch (s.type) {
    case "integer": return "int";
    case "number": return "number";
    case "boolean": return "bool";
    case "string": return s.format === "date" ? "date" : s.format === "date-time" ? "datetime" : "string";
    case "array": return depth < 2 ? `${typeOf(spec, s.items, depth + 1)}[]` : "any";
    default: return "any";
  }
}
...
function operations(spec: Json, o: OpenApiOptions, prefix: string): Op[] {
  ...
  for (const [path, item] of Object.entries<Json>(spec.paths ?? {})) {
    for (const method of ["get", "post", "put", "patch", "delete"]) {
      ...
      const base = typeof op.operationId === "string" && /^[A-Za-z][A-Za-z0-9_]*$/.test(op.operationId) ? op.operationId : slug(op.operationId ?? `${method}_${path}`);
      let name = `${prefix}.${base}`;
      for (let i = 2; used.has(name); i++) name = `${prefix}.${base}${i}`;
      used.add(name);
      const params: ParamSchema = {};
      const pathParams: string[] = [], queryParams: string[] = [];
      for (const raw of [...(item.parameters ?? []), ...(op.parameters ?? [])]) {
        ...
        params[p.name + (p.required || p.in === "path" ? "" : "?")] = describe(typeOf(spec, p.schema), p.description);
      }
      ...
      if (schema) {
        if (schema.type === "object" || schema.properties) {
          ...
          for (const [k, v] of Object.entries<Json>(schema.properties ?? {})) {
            ...
            bodyKeys.push(k);
            params[k + (req.has(k) ? "" : "?")] = describe(typeOf(spec, v), resolve(spec, v)?.description);
          }
        } else {
          bodyKeys = "whole";
          params[body.required ? "body" : "body?"] = describe(typeOf(spec, schema), "request body");
        }
      }
...
export function fromOpenAPI(spec: Json, o: OpenApiOptions = {}): Service {
  ...
  if (!/^https?:\/\//.test(baseUrl)) throw new Error("OpenAPI adapter needs an absolute baseUrl (spec has no absolute servers[0].url)");
  ...
  const svc = service({
    id: o.id ?? host,
    name: title,
    summary: (spec.info?.description ?? `${title} (via OpenAPI)`).replace(/\s+/g, " ").trim().slice(0, 200),
    trust: o.trust ?? [],
  });
  ...
  const build = (op: Op, params: Json) => {
    ...
    for (const k of op.pathParams) {
      if (params[k] === undefined) throw new ParleyError("invalid_params", `missing path parameter \`${k}\``);
      path = path.replace(`{${k}}`, encodeURIComponent(String(params[k])));
    }
    const url = new URL(baseUrl + path);
    for (const k of op.queryParams) if (params[k] !== undefined) url.searchParams.set(k, Array.isArray(params[k]) ? params[k].join(",") : String(params[k]));
    let body: unknown = undefined;
    if (op.bodyKeys === "whole") body = params.body;
    else if (op.bodyKeys) body = Object.fromEntries(op.bodyKeys.filter((k) => params[k] !== undefined).map((k) => [k, params[k]]));
    return { url, body };
  };
  ...
  const call = async (method: string, url: URL, body: unknown) => {
    ...
    try {
      res = await doFetch(url, {
        ...
        headers: { accept: "application/json", ...(body !== undefined ? { "content-type": "application/json" } : {}), ...o.headers },
        body: body !== undefined ? JSON.stringify(body) : undefined,
        signal: AbortSignal.timeout(o.timeoutMs ?? 30_000),
      });
    } catch (e) {
      throw new ParleyError("unavailable", `upstream request failed: ${(e as Error).message}`, { retry: 5 });
    }
    ...
    if (!res.ok) {
      const code = res.status === 404 ? "not_found" : res.status === 401 || res.status === 403 ? "forbidden" : res.status === 409 ? "conflict" : res.status === 429 ? "limit" : res.status < 500 ? "invalid_params" : "unavailable";
      throw new ParleyError(code, `upstream ${res.status}: ${typeof data === "string" ? data.slice(0, 200) : JSON.stringify(data).slice(0, 200)}`);
    }
    return data;
  };
...
export async function loadOpenAPI(source: string): Promise<Json> {
  ...
  try {
    ...
    throw new Error("the OpenAPI document must be JSON (convert YAML first, e.g. with `npx js-yaml spec.yaml > spec.json`)");
  }
}
