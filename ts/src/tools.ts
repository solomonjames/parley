...
export const TOOLS = [
  ...
  { name: "parley_undo", description: "Undo a receipt within its undo window.", inputSchema: obj({ service: str, receipt: str }, ["service", "receipt"]) },
];

/** Ask the human to approve `shown` (the proposal's Lens). Resolve true only on explicit approval. */
export type Approver = (req: { service: string; shown: string; reason: string }) => Promise<boolean>;
...
export async function createToolHost(clients: Client[], approve?: Approver): Promise<ToolHost> {
  ...
  for (const c of clients) {
    try {
      ...
      problems.push(`(a service could not be reached: ${(e as Error).message})`);
    }
  }
  const none = "No Parley services are configured. Tell the user to run `npx parley-protocol add <url>` (or `npx parley-protocol setup`) and restart.";
  const instructions = INSTRUCTIONS + (briefs.length ? briefs.join("\n\n") : none) + (problems.length ? "\n\n" + problems.join("\n") : "");
  ...
  async function askHuman(consent: ConsentRequest, err: ErrorReply, c: Client, p: Proposal): Promise<string | null> {
    ...
    if (!approve || !principal || principal.public !== consent.principal || !c.key) return null;
    const shown = lens({ parley: 1, id: "-", re: "-", kind: "PROPOSALS", proposals: [p] }).split("\n").slice(1).join("\n");
    if (!(await approve({ service: consent.service, shown, reason: err.message }))) return null;
  ...
  return { instructions, services: [...services.keys()], call, close: () => clients.forEach((c) => c.close()) };
}
