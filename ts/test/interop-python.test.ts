...
const ready = hasUv && existsSync(`${pyDir}/examples/serve.py`) && !process.env.SKIP_INTEROP;
const TCP = 17457, HTTP = 18457;

describe.skipIf(!ready)("interop: TS client → Python service", async () => {
  const principal = await P.keyPair(), agent = await P.keyPair();
  let proc: ChildProcess;
  beforeAll(async () => {
    proc = spawn("uv", ["run", "python", "examples/serve.py"], { cwd: pyDir, env: { ...process.env, PARLEY_TRUST: principal.public, PARLEY_PORT: String(TCP), PARLEY_HTTP_PORT: String(HTTP) }, stdio: "ignore" });
    for (let i = 0; i < 100; i++) {
      try {
        await (await fetch(`http://127.0.0.1:${HTTP}/.well-known/parley`)).json();
        return;
      } catch {
  ...
  for (const url of [`parley://127.0.0.1:${TCP}`, `http://127.0.0.1:${HTTP}/parley`]) {
    it(`full flow over ${url.split(":")[0]}`, async () => {
      const grant = await P.issueGrant({ principal, to: agent.public, caveats: [{ svc: ["calendar.example"] }] });
      ...
      const p = await c.intent("calendar.reschedule", { event: "Ana", ...q.options[0].params, day: "2031-03-04" });
      ...
      const auto = await c.intent("calendar.reschedule", { event: "e2", day: "2031-03-05" }, { auto: true });
