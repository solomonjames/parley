...
async function setup(caveats: P.Caveat[] = []) {
  ...
  const client = new P.Client(P.local(billing({ trust: [principal.public] })), { key: agent.seed, grants: [grant] });
  return { client };
}
...
describe("billing example (service design guide)", () => {
  it("resolves people by name, and CLARIFYs when ambiguous", async () => {
    ...
    const one = await client.ask("billing.customer", { who: "ana.ruiz@acme.co" });
  ...
  it("a per-action cap makes large refunds need consent", async () => {
    const { client } = await setup([{ risk: "medium" }, { per: { max: 2000, currency: "USD" } }]);
  ...
  it("scheduling a plan change auto-commits under a low-risk grant, and undoes", async () => {
    ...
    const p = await client.intent("billing.change_plan", { who: "Dana", plan: "pro" });
