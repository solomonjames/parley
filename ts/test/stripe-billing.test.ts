...
function fakeStripe() {
  ...
  const charges = [
    { id: "ch_2", customer: "cus_chen", amount: 4900, amount_refunded: 0, currency: "usd", status: "succeeded", created: now - 16 * D },
    { id: "ch_1", customer: "cus_chen", amount: 4900, amount_refunded: 0, currency: "usd", status: "succeeded", created: now - 46 * D },
  ];
  const subs = [{ id: "sub_chen", customer: "cus_chen", status: "active", cancel_at_period_end: false,
    items: { data: [{ current_period_start: now - 16 * D, current_period_end: now + 14 * D, price: { id: "price_pro", nickname: "pro" } }] } }];
  const calls: { method: string; path: string; body: string; key: string | null }[] = [];

  const fetch = async (url: string | URL | Request, init?: RequestInit) => {
    const u = new URL(String(url)), method = init?.method ?? "GET", body = String(init?.body ?? "");
    const headers = new Headers(init?.headers);
    calls.push({ method, path: u.pathname + u.search, body, key: headers.get("idempotency-key") });
    const ok = (x: unknown) => new Response(JSON.stringify(x), { status: 200 });
    const p = u.pathname.replace("/v1", ""), form = new URLSearchParams(body);
    if (p === "/customers/search") {
      // Enough of Stripe's query language for `name:"x" OR email:"x"`: every word must appear.
      const words = JSON.parse(u.searchParams.get("query")!.split(" OR ")[0].slice(5)).toLowerCase().split(/\s+/);
      return ok({ data: customers.filter((c) => words.every((w: string) => `${c.name} ${c.email}`.toLowerCase().split(/[^a-z0-9]+/).includes(w) || c.email === w)) });
    }
    if (p.startsWith("/customers/")) {
      const c = customers.find((x) => x.id === p.split("/")[2]);
      return c ? ok(c) : new Response(JSON.stringify({ error: { message: "No such customer" } }), { status: 404 });
    }
    if (p === "/charges") return ok({ data: charges.filter((c) => c.customer === u.searchParams.get("customer")) });
    if (p === "/subscriptions") return ok({ data: subs.filter((s) => s.customer === u.searchParams.get("customer") && s.status === "active") });
    if (p === "/refunds") {
...
async function setup(caveats: P.Caveat[] = []) {
  ...
  const client = new P.Client(P.local(stripeBilling({ key: "sk_test_x", trust: [principal.public], fetch: stripe.fetch })), { key: agent.seed, grants: [grant] });
...
describe("Stripe-backed billing (the guide's full example)", () => {
  ...
  it("ambiguous names CLARIFY; a refund offers full or unused, and is never auto-committed", async () => {
    ...
    expect((await client.intent("billing.refund", { who: "Ana" })).kind).toBe("CLARIFY");
    const r = await client.intent("billing.refund", { who: "Chen" }, { auto: true });
    ...
    expect(call.body).toMatch(/^charge=ch_2&amount=\d+&reason=requested_by_customer$/);
  ...
  it("cancel at period end is undone with the inverse REST call", async () => {
    ...
    const r = await client.intent("billing.cancel", { who: "chen@wei.studio" }, { auto: true });
    ...
    expect(stripe.calls.filter((c) => c.method === "POST").map((c) => c.body)).toEqual(["cancel_at_period_end=true", "cancel_at_period_end=false"]);
  });
