...
[Method, raw payloads and caveats →](https://github.com/solomonjames/parley/blob/main/bench/RESULTS.md)
...
- **Tokens aren't the point of the preview step.** An early version of this benchmark (before auto-commit) had Parley *losing* on multi-step tasks, because proposals cost a round trip. That led to [policy-gated auto-commit](https://github.com/solomonjames/parley/blob/main/SPEC.md#431-policy-gated-auto-commit) and to hoisting shared proposal attributes in Lens.
...
infrastructure requires it. Its semantics just don't depend on it. [More →](https://github.com/solomonjames/parley/blob/main/docs/design.md)
...
with caveats a service can check offline. [More →](https://github.com/solomonjames/parley/blob/main/docs/design.md#grants)
...
by regression tests ([details](https://github.com/solomonjames/parley/blob/main/docs/design.md#security-review)). The reference services keep state in memory. The protocol surface is deliberately small. Before 1.0: revocation lists,
multi-party atomic commits (`HOLD` across services), and a QUIC transport. See the
[roadmap](https://github.com/solomonjames/parley/blob/main/docs/design.md#roadmap). Feedback on the spec is the most valuable
