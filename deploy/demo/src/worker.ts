...
interface Env { DEMO: DurableObjectNamespace<ParleyDemo> }
const SERVICES = { calendar: () => calendar({ trust: () => true }), shop: () => shop({ trust: () => true }) };
...
export default {
  async fetch(req: Request, env: Env): Promise<Response> {
    ...
    if (!(name in SERVICES)) return new Response(HOME.replaceAll("<this-origin>", url.origin), { headers: { "content-type": "text/plain; charset=utf-8" } });
    let key = "public";
    let body: string | undefined;
    if (req.method === "POST") {
      body = await req.text();
      if (body.length > MAX_FRAME) return new Response("frame exceeds 1 MiB", { status: 413 });
      try {
