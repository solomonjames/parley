...
2. **Consistency across services.** A model that has read one Parley receipt can read them all.
3. **Budgets that mean something.** A budget constrains the text the model actually reads.

Lens is designed to be read cold. In [a real session](/reference/claude-session), a model with no Parley documentation read it correctly on the first try.
