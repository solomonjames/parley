...
let seed = 0x9e3779b9;
globalThis.crypto.getRandomValues = (<T extends ArrayBufferView | null>(a: T): T => {
  const u8 = new Uint8Array((a as any).buffer, (a as any).byteOffset, (a as any).byteLength);
  for (let i = 0; i < u8.length; i++) { seed ^= seed << 13; seed ^= seed >>> 17; seed ^= seed << 5; u8[i] = seed & 0xff; }
  return a;
}) as any;
...
interface Task { name: string; note?: string; rest: (J: (v: unknown) => string) => Run; parley: Run }
const DAY = new Date(Date.now() + 3 * 86400e3).toISOString().slice(0, 10);

async function tasks(): Promise<Task[]> {
  const out: Task[] = [];
  const menuRow = (m: any) => ({ sku: m.sku, name: m.name, price_usd: m.usd, calories: m.cal, protein_g: m.protein });
  ...
  //    separate row, an outcome-level endpoint, so the protocol isn't credited for API design.
  {
    ...
    const parley = { calls: [{ args: { name: "parley_intent", arguments: intentArgs }, result: r.lens }] };
    out.push({
    ...
    out.push({
      ...
      rest: (J) => ({ calls: [{ args: { name: "reschedule_event", arguments: { query: "1:1 Ana", day: DAY } }, result: J(moved) }] }),
      parley,
    });
  }
  ...
  // 2. Find vegan meals under 700 kcal and order four.
  {
    ...
    const order = { id: "o1001", status: "placed", deliver: DAY, items: lines, subtotal, delivery_fee: subtotal >= 50 ? 0 : 5.99, total: Math.round((subtotal + (subtotal >= 50 ? 0 : 5.99)) * 100) / 100, currency: "USD", cancellable_until: "2026-01-01T02:00:00Z" };
    ...
    out.push({
      ...
      rest: (J) => ({ calls: [
        { args: { name: "search_meals", arguments: { tag: "vegan", max_cal: 700 } }, result: J(found.map(menuRow)) },
  ...
  // 3. Read the whole 60-item menu (same content both ways; pure encoding cost).
  {
    ...
    out.push({
      ...
      rest: (J) => ({ calls: [{ args: { name: "search_meals", arguments: {} }, result: J(all.map(menuRow)) }] }),
  ...
  //    number of items Parley returned. Same items both ways; Parley also says what's left.
  {
    ...
    out.push({
      ...
      rest: (J) => ({ calls: [{ args: { name: "search_meals", arguments: { limit: shown } }, result: J(all.slice(0, shown).map(menuRow)) }] }),
      parley: { calls: [{ args: { name: "parley_ask", arguments: { service: "shop.example", capability: "shop.search", params: {}, budget: 800 } }, result: a.lens }] },
    });
  }
  return out;
}
...
const minJ = (v: unknown) => JSON.stringify(v), prettyJ = (v: unknown) => JSON.stringify(v, null, 2);

log(`# Parley vs REST-style MCP — token benchmark\n`);
log(`Tokenizer: o200k_base (gpt-tokenizer). Claude's tokenizer differs; the ratios are what matter. Both sides serve the same data. Ids are seeded, so runs are reproducible.\n`);
log(`**Total input** counts what you pay for: each model turn re-reads the tool definitions plus the conversation so far (calls and results), and there's one final turn to answer.\n`);
log(`Tool definitions in context every turn: REST MCP **${tok(restDefs)}** tokens (${REST_TOOLS.length} tools) vs Parley **${tok(parleyDefs)}** (${PARLEY_TOOLS.length} generic tools + service briefs).\n`);
const T = await tasks();
log(`| Task | Calls (REST → Parley) | Total input: REST minified JSON | REST pretty JSON | Parley | Saved vs minified | vs pretty |`);
log(`|---|---|---|---|---|---|---|`);
let SM = 0, SP = 0, SX = 0;
for (const t of T) {
  const m = cost(restDefs, t.rest(minJ)), pr = cost(restDefs, t.rest(prettyJ)), px = cost(parleyDefs, t.parley);
  if (!t.name.includes("outcome-level")) (SM += m.cumulative), (SP += pr.cumulative), (SX += px.cumulative);
  log(`| ${t.name} | ${m.calls} → ${px.calls} | ${n(m.cumulative)} | ${n(pr.cumulative)} | ${n(px.cumulative)} | **${pct(m.cumulative, px.cumulative)}** | ${pct(pr.cumulative, px.cumulative)} |`);
}
log(`| **All tasks** (CRUD reschedule row) | | ${n(SM)} | ${n(SP)} | ${n(SX)} | **${pct(SM, SX)}** | ${pct(SP, SX)} |\n`);
log(`Result tokens read, per task (minified REST → Parley): ${T.map((t) => `${cost(restDefs, t.rest(minJ)).read} → ${cost(parleyDefs, t.parley).read}`).join(" · ")}\n`);

log(`## What the model actually reads\n`);
const cr = T[0];
log(`### Reschedule, REST CRUD (3 calls, pretty JSON)\n\n\`\`\`json\n${cr.rest(prettyJ).calls.map((c) => c.result).join("\n\n")}\n\`\`\`\n`);
log(`### Reschedule, Parley (1 call, auto-commit)\n\n\`\`\`\n${cr.parley.calls.map((c) => c.result).join("\n\n")}\n\`\`\`\n`);
log(`What the tokens don't show: the Parley agent acted only because the principal's grant allows low-risk, undoable changes, and it got back exactly what happened with a 24h undo window. In the auto-commit case the *service* chose the slot (the first free one), just like the REST outcome endpoint. An agent that wants to choose omits \`auto\` and gets three proposals instead.`);
