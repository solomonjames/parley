...
const roundDown = (secs: number) =>
  secs >= 86400 ? Math.floor(secs / 86400) * 86400 : Math.max(0, Math.floor(secs / 3600) * 3600);
...
export function stripeBilling(opts: { key: string; trust: string[]; fetch?: typeof fetch }) {
  ...
  async function stripe(
    ...
    if (res.status === 429)
      throw new ParleyError("limit", "Stripe is rate limiting; retry shortly", { retry: 2 });
  ...
  async function findCustomers(who: string): Promise<any[]> {
    ...
    const query = new URLSearchParams({ query: `name:${q} OR email:${q}`, limit: "5" });
  ...
  async function one(who: string, then: (c: any) => Promise<Plan | Plan[]>) {
    ...
    if (!m.length)
      throw new ParleyError("not_found", `no customer matches ${JSON.stringify(who)}`, {
  ...
  const charges = async (c: any) =>
    (await stripe("GET", `/charges?${new URLSearchParams({ customer: c.id, limit: "5" })}`)).data;
      ...
      .ask("billing.customer", {
        ...
        async run({ params }) {
          ...
          return {
            ...
            ...(sub
              ? { [sub.cancel_at_period_end ? "cancels" : "renews"]: day(period(sub).end) }
              : {}),
        ...
        plan: ({ params }) =>
          one(params.who, async (c) => {
            ...
            const ch = params.payment
              ...
              : chs.find((x: any) => x.status === "succeeded" && x.amount_refunded < x.amount);
            if (!ch)
              throw new ParleyError(
                "not_found",
            ...
            const refund = (amount: number, why: string): Plan => ({
              ...
              apply: () =>
                stripe(
                  "POST",
                  "/refunds",
                  { charge: ch.id, amount: String(amount), reason: "requested_by_customer" },
            ...
            if (params.amount != null) {
              ...
              if (amount <= 0 || amount > left)
                throw new ParleyError(
                  ...
                  {
                    fix: [
                      fix(`refund the rest (${amt(left, ch.currency)})`, { amount: left / 100 }),
                    ],
                  },
                );
            ...
            if (sub && ch.id === chs[0]?.id) {
              ...
              if (unused > 0 && unused < left)
                plans.push(refund(unused, `unused ${Math.round((end - now) / 86400)} days`));
            }
            return plans;
          }),
      })
