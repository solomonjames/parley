...
const sh = shop({ trust });
await Promise.all([
  listen(cal, { port: 7447, host }), serveHttp(cal, { port: 8447, host }),
  listen(sh, { port: 7449, host }), serveHttp(sh, { port: 8449, host }),
]);
console.error(`parley examples up · calendar parley://${host}:7447 http://${host}:8447/parley · shop parley://${host}:7449 http://${host}:8449/parley · trusting ${trust.length} principal(s)`);
