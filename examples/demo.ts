...
import { consentGrant, delegateGrant, est, issueGrant, keyPair, lens, type Answer, type Proposal } from "parley-protocol";
import { connect, listen } from "parley-protocol/node";
...
const wire = (verb: string, detail: string) => console.log(k(`   → ${verb}`, c.cyan) + k(` ${detail}`, c.dim));
const show = (text: string) => {
  console.log(text.split("\n").map((l) => "   " + k("│ ", c.dim) + l).join("\n"));
  console.log(k(`   └ ${est(text)} tokens`, c.dim));
};
const human = k("👤 human", c.yel), agentName = k("🤖 agent", c.mag), subName = k("🤖 sub-agent", c.mag);

// ── setup: two services on real sockets ──────────────────────────────────────
const james = await keyPair(), agent = await keyPair(), helper = await keyPair();
...
const policy = await issueGrant({
  principal: james, to: agent.public,
  caveats: [{ svc: ["calendar.example", "shop.example"] }, { risk: "low" }, { per: { max: 4000, currency: "USD" } }, { spend: { max: 10000, currency: "USD" } }, { exp: Math.floor(Date.now() / 1000) + 8 * 3600 }],
});
show(`grant ${policy.slice(0, 28)}… (${policy.length} bytes, signed Ed25519)\n  services: calendar.example, shop.example\n  risk ≤ low · ≤ 40.00 USD per action · ≤ 100.00 USD total · expires in 8h`);

const cal = await connect(calUrl, { key: agent.seed, grants: [policy], name: "demo-agent" });
const sh = await connect(shopUrl, { key: agent.seed, grants: [policy], name: "demo-agent" });

say(agentName, "discovers what the calendar can do:");
wire("HELLO", calUrl);
show((await cal.hello()).lens);

say(agentName, `"move my 1:1 with Ana to ${day(3)}", said with intent instead of CRUD calls:`);
wire("INTENT", `calendar.reschedule {event:"Ana", day:"${day(3)}"} auto`);
const q = await cal.intent("calendar.reschedule", { event: "Ana", day: day(3) }, { auto: true });
show(q.lens);

if (q.kind === "CLARIFY") {
  say(agentName, "picks option 1. The service knows it's low-risk and undoable, and the policy allows that, so it commits in the same round trip:");
  wire("INTENT", `calendar.reschedule ${JSON.stringify({ ...q.options[0].params, day: day(3) })} auto`);
  const r = await cal.intent("calendar.reschedule", { event: "Ana", day: day(3), ...q.options[0].params }, { auto: true });
  show(r.lens);
...
say(agentName, `orders 4 meals. It's over the 40.00 USD per-action limit, so no auto-commit, just proposals:`);
wire("INTENT", `shop.order {items:[m005×2, m007×2], deliver:"${day(2)}"} auto`);
const order = await sh.intent("shop.order", { items: [{ sku: "m005", qty: 2 }, { sku: "m007", qty: 2 }], deliver: day(2) }, { auto: true });
show(order.lens);
...
say(agentName, "hands a sub-agent a narrower, read-only slice of its authority, offline and without asking the service:");
const narrowed = await delegateGrant(policy, { holder: agent, to: helper.public, caveats: [{ verbs: ["HELLO", "ASK", "INTENT"] }, { can: ["shop.search", "shop.order"] }] });
const sub = await connect(shopUrl, { key: helper.seed, grants: [narrowed], name: "sub-agent" });
say(subName, "tries to place an order anyway:");
const sp = await sub.intent("shop.order", { items: [{ sku: "m001", qty: 1 }], deliver: day(2) });
